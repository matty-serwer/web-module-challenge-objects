...
const Pizza = createMenuItem("Pizza", 4, "Dinner");
console.log(Pizza);

/* Task 1b: use your function to create 3 more menu items. You may add any items to the menu that you'd like */

const chickenWings = createMenuItem("Chicken Wings", 14, "Dinner");
const tacos = createMenuItem("Tacos", 12, "Lunch");
const beer = createMenuItem("Pint Lager", 7, "Drinks");
...
reviews.push({
  ...
  rating: 5,
  feedback:
    "This is the best club of all time. I loved the wall to wall Jello!",
});
...
function getReviewByRating(reviews, range) {
  const reviewsInRange = [];

  for(let i = 0; i < reviews.length; i++) {
    if(reviews[i].rating >= range && reviews[i].rating < range + 1) {
...
function getLongReviews(reviews) {
  ...
  for(let i = 0; i < reviews.length; i++) {
    ...
    if(reviews[i].feedback) {
      wordCount++;
      for(let x = 0; x < reviews[i].feedback.length; x++) {
        if(reviews[i].feedback.charAt(x) == ' ') {
            wordCount++;
        }
      } // go through the string in feedback for spaces
      if(wordCount > 15) {
        longReviews.push(reviews[i])
      } // if wordCount over 15
    }  // if there is feedback
  } // for length of review list
  return longReviews;
} // function

console.log(getLongReviews(reviews));

// this one melted my BRAIN! ^^^^
...
const myFerrari = carMarker(2000);

let newOdometer = myFerrari.drive(35);
